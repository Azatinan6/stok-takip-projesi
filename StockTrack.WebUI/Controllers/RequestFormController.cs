...
namespace StockTrack.WebUI.Controllers
{
    ...
    public class RequestFormController : Controller
    {
        ...
        {
            // Lokasyonlar
            var locations = await _appDbContext.LocationLists.AsNoTracking().Where(x => !x.IsDeleted && x.IsActive).Select(x => new SelectListItem { Value = x.Id.ToString(), Text = x.Name }).ToListAsync();

            // Depolar
            var repos = await _appDbContext.MainRepoLocations.AsNoTracking().Where(x => !x.IsDeleted && x.IsActive).Select(x => new SelectListItem { Value = x.Id.ToString(), Text = x.Name }).ToListAsync();
            // kullanıcılar
            var persons = await _userManager.Users.Where(x => x.IsActive && !x.IsDeleted).Select(x => new SelectListItem { Value = x.Id.ToString(), Text = x.NameSurname }).ToListAsync();
        ...
        public async Task<IActionResult> GetCategoriesByRepo(int mainRepoId)
        {
            var cats = await (from pml in _appDbContext.ProductMainRepoLocations.AsNoTracking()
                     ...
                     .GroupBy(x => x.id) // Aynı id'ye sahip kayıtları grupla
                     .Select(g => g.First()) // Her gruptan ilk elemanı seç (bu her zaman benzersiz olacaktır)
                     .ToListAsync();
        ...
        {
            // Depodaki giriş (o depoda bu üründen kaç adet var)
            ...
            // Aynı depodan bu üründen dağıtılmış toplam miktar
            var distributed = await (from rp in _appDbContext.RequestProducts.AsNoTracking()
                                     ...
                                     where rp.ProductId == productId && rf.MainRepoLocationId == mainRepoId && !rf.IsDeleted && !rfd.IsDeleted && rfd.StatusId != (int)EnumStatusType.İptal
        ...
        public async Task<IActionResult> Create(CreateRequestFormDto dto)
        {
            ...
            // ADIM 2) Görünüm verilerini her durumda doldur
            var locations = await _appDbContext.LocationLists.AsNoTracking()
                .Where(x => !x.IsDeleted && x.IsActive)
                .Select(x => new SelectListItem { Value = x.Id.ToString(), Text = x.Name + " - " + x.Address })
                .ToListAsync();
            ...
            // ADIM 3) Sunucu Tarafı Doğrulamaları (güvenlik)

            //  Zorunlu alanlar
            if (dto.MainRepoId <= 0)
                ModelState.AddModelError("", "Depo seçimi zorunludur.");
            if (dto.LocationId <= 0)
                ModelState.AddModelError("", "Lokasyon seçimi zorunludur.");
            if (dto.TypeId <= 0)
                ModelState.AddModelError("", "Talep türü zorunludur.");

            if (dto.TypeId == (int)EnumRequestType.Kargo)
            {
                if (string.IsNullOrWhiteSpace(dto.ReceiverFullName))
                    ModelState.AddModelError("", "Kargo için alıcı adı zorunludur.");
            }
            else if (dto.TypeId == (int)EnumRequestType.Kurulum || dto.TypeId == (int)EnumRequestType.Servis)
            {
                if (dto.InstallationDate == null)
                    ModelState.AddModelError("", "Kurulum/Servis için tarih zorunludur.");

                if (dto.Persons == null || dto.Persons.Count == 0)
                    ModelState.AddModelError("", "Kurulum/Servis için en az bir personel eklenmelidir.");
            }

            // Ürün satır kontrolleri (miktar, tekrar, stok aşımı)
            ...
            foreach (var it in items)
            {
                ...
                if (!duplicate.Add(it.ProductId))
                    ModelState.AddModelError("", "Aynı ürünü birden fazla kez ekleyemezsiniz.");

                // Stok kontrolü: depodaki miktar - dağıtılmış miktar
                ...
                var distributed = await (from rp in _appDbContext.RequestProducts.AsNoTracking()
                                         join rf in _appDbContext.RequestForms.AsNoTracking() on rp.RequestFormId equals rf.Id
                                         where rp.ProductId == it.ProductId && rf.MainRepoLocationId == dto.MainRepoId && !rf.IsDeleted
                                         select (int?)rp.Quantity).SumAsync() ?? 0;

                var remaining = Math.Max(0, inRepoQty - distributed);
                if (it.Quantity > remaining)
                    ModelState.AddModelError("", $"İstenen miktar stoktan fazla: ÜrünId {it.ProductId} için istenen {it.Quantity}, kalan {remaining}.");
            }
            ...
            try
            {
                ...
                {
                    RequestFormId = requestForm.Id,
                    StatusId = (int)EnumStatusType.Talep, //Talep oluşturma
                    CreatedBy = currentUser.NameSurname,
                    CreatedDate = now,
                    ModifiedDate = now,
                    IsDeleted = false,
                ...
                if (dto.TypeId == (int)EnumRequestType.Kurulum || dto.TypeId == (int)EnumRequestType.Servis)
                {
                    ...
                    {
                        requestFormDetail.StatusId = (int)EnumStatusType.Tamamlandı;//sahada gerçekleşen kurulum servisleri
                    ...
                    TempData["SuccessMessage"] = "Kurulum/Servis talebiniz oluşturuldu.";
                }
                else if (dto.TypeId == (int)EnumRequestType.Kargo)
                {
                    ...
                    TempData["SuccessMessage"] = "Kargo talebiniz oluşturuldu.";
                }
            ...
            catch
            {
                ...
                TempData["ErrorMessage"] = "Talep oluşturulurken hata oluştu, işlemler geri alındı.";
                return View(dto);
            }
        }
        ...
        public async Task<IActionResult> SaveRequestFormStatus(SaveRequestFormStatusDto dto)
        {
            if (!ModelState.IsValid)
            {
                //  tüm hata mesajlarını tek bir string'de birleştiriyoruz.
